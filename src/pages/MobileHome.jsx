        ...
        <main className="pt-[68px] overflow-x-hidden">
            {(data.home_layout || []).map((section) => {
                    ...
                    return (
                        <section key={section.id} className="mb-10">
                            <div className="relative w-full aspect-[3/4] overflow-hidden group">
                                <img src={data.banners?.[0]?.image || "https://lh3.googleusercontent.com/aida-public/AB6AXuBdso03s2h1V8LfKMtiajCAAMOq8t0GfkW8B4cTVQeQO29ytbjJfXzkkYkfIOwWCOMCyN_PV81XZfRNn6deAxRSdOrSRh8P3ZvBFwd5phxoW3KWlygiW6vJGPAaO8j2S1IvWzuKpbwF-UGFHIyUXuU6xzn3lsa9pQxb03y0T7rXzXgFH4K00IYnefqSNR-SIYCJMbdGHoE-_DzEZcmITZZwHfzHQRwA-4qFB6KRIBMNNdZTdwXrkozGQke_Q_FOZdSfHWE5FaCosrgd"} 
                                     alt="Hero" 
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-background via-background/20 to-transparent flex flex-col justify-end p-6">
                                    ...
                                    <button className="bg-[#22c55e] text-black font-black py-3 px-8 rounded-sm w-fit text-[11px] tracking-widest uppercase" onClick={() => navigate(data.banners?.[0]?.link || '/')}>Explorar</button>
                                </div>
                            </div>
                        </section>
                    );
                ...
                if (section.type === 'categories') {
                    return (
                        <section key={section.id} className="mb-14 px-4">
                            <div className="flex items-baseline justify-between mb-5">
                               <h3 className="text-[11px] font-bold uppercase tracking-[.3em] text-white/50 px-1 font-['Inter']">{section.title || 'Navegar por'}</h3>
                            </div>
                            <AutoCarousel speed={0.3} gap={12}>
                                {data.categories.map(cat => (
                                    <div 
                                      ...
                                      className="min-w-[100px] shrink-0 flex flex-col items-center justify-center p-4 bg-surface-container-low rounded-md aspect-square text-center active:bg-surface-container-high transition-colors cursor-pointer"
                                    >
                ...
                if (section.type === 'collection') {
                    ...
                    return (
                        <section key={section.id} className="mb-12 px-4">
                            <div className="flex justify-between items-end mb-8 px-1">
                                <div>
                                    <h3 className="text-2xl font-black tracking-tighter uppercase leading-none text-white">{section.title}</h3>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {filtered.map(p => (
                                    <div key={p.id || p._id} className="group flex flex-col gap-3 cursor-pointer" onClick={() => navigate(`/produto/${p.id || p._id}`)}>
                                        <div className="relative aspect-square rounded-md overflow-hidden bg-surface-container-low">
                                            <img src={p.image} alt={p.name} className="w-full h-full object-cover active:scale-105 transition-transform" />
                                            {p.featured && (
                                                <div className="absolute top-2 left-2">
                                                    <span className="bg-[#22c55e] text-black text-[9px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-widest">Destaque</span>
                                                </div>
                                            )}
                                        </div>
